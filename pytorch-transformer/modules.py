...
class embedding(nn.Module):
    ...
    def forward(self, inputs):
        if self.zeros_pad:
            ...
            self.padding_idx = -1
        outputs = self.lookup_table[inputs]#self._backend.Embedding.apply(
...
class positional_encoding(nn.Module):
    ...
    def forward(self, inputs):
        ...
        position_ind =torch.unsqueeze(torch.arange(0, T), 0).repeat(N, 1).long().cuda()
        ...
        outputs = lookup_table[position_ind]#self._backend.Embedding.apply(
